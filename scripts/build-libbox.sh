...
if grep -q "FRKN patch: re-seed randomized" "$FP_FILE" 2>/dev/null; then
  ...
  say "pwd=$(pwd) | sed: $(sed --version 2>&1 | head -1)"
  say "before patch:"; grep -n "randomizedFingerprint" "$FP_FILE" || true
  ...
  if ! sed -i 's|return randomizedFingerprint, nil|frknFP := randomizedFingerprint\n\t\tfrknFP.Seed, _ = utls.NewPRNGSeed()\n\t\treturn frknFP, nil // FRKN patch: re-seed randomized|' "$FP_FILE"; then
    die "sed failed on $FP_FILE (exit $?)"
  fi
  say "after patch:"; grep -n "FRKN patch\|frknFP\|randomizedFingerprint" "$FP_FILE" || true
