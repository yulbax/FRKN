...
class LinkParserTest {
    ...
    fun parsesVlessTlsProfile() {
        ...
        assertEquals("vless", parsed.type)
    ...
    fun parsesPlainAndBase64Subscriptions() {
        ...
        assertEquals(1, SubscriptionFetcher.parseBody(link).size)
        assertEquals(1, SubscriptionFetcher.parseBody(encoded).size)
        assertEquals("One", SubscriptionFetcher.parseBody(encoded).single().name)
    }
}
